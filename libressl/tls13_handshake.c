...
static int
fuel_fuzz_handshake_msg(struct tls13_ctx *ctx)
{
	struct shared_buffer shbuf;
	CBS cbs;

	if (ctx == NULL || ctx->ssl == NULL || ctx->ssl->server ||
	    ctx->hs_msg == NULL)
		return 1;

	tls13_handshake_msg_data(ctx->hs_msg, &cbs);
	if (CBS_len(&cbs) == 0 || CBS_len(&cbs) > sizeof(shbuf.data) ||
	    CBS_len(&cbs) > UINT16_MAX)
		return 1;

	memcpy(shbuf.data, CBS_data(&cbs), CBS_len(&cbs));
	shbuf.len = (uint16_t)CBS_len(&cbs);
	shbuf.type = tls13_handshake_msg_type(ctx->hs_msg);

	fuzz_plaintext(&shbuf);

	return tls13_handshake_msg_replace(ctx->hs_msg, shbuf.data, shbuf.len);
}
#endif
...
#ifdef FUEL_FUZZ
		if (!fuel_fuzz_handshake_msg(ctx))
			return TLS13_IO_FAILURE;
#endif
	}
