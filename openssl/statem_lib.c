...
# define FUEL_TLS_EXT_KEY_SHARE 0x0033u

static int fuel_get_clienthello_extensions(const unsigned char *msg,
                                           size_t msglen,
                                           size_t *exts_len_offset,
                                           size_t *exts_offset,
                                           size_t *exts_len)
{
    size_t offset = SSL3_HM_HEADER_LENGTH + 2 + SSL3_RANDOM_SIZE;
    size_t session_id_len;
    size_t cipher_suites_len;
    size_t compression_methods_len;

    if (msg == NULL || msglen < offset + 1)
        return 0;

    session_id_len = msg[offset++];
    if (msglen < offset + session_id_len + 2)
        return 0;
    offset += session_id_len;

    cipher_suites_len = ((size_t)msg[offset] << 8) | msg[offset + 1];
    offset += 2;
    if (msglen < offset + cipher_suites_len + 1)
        return 0;
    offset += cipher_suites_len;

    compression_methods_len = msg[offset++];
    if (msglen < offset + compression_methods_len + 2)
        return 0;
    offset += compression_methods_len;

    *exts_len_offset = offset;
    *exts_len = ((size_t)msg[offset] << 8) | msg[offset + 1];
    *exts_offset = offset + 2;

    if (msglen < *exts_offset + *exts_len)
        return 0;

    return 1;
}

static int fuel_find_clienthello_extension(const unsigned char *msg,
                                           size_t msglen,
                                           ...
                                           int *found)
{
    ...
    *found = 0;
    if (!fuel_get_clienthello_extensions(msg, msglen, exts_len_offset,
    ...
    while (cursor + 4 <= end) {
        size_t current_len = ((size_t)msg[cursor + 2] << 8) | msg[cursor + 3];
        unsigned int current_type = ((unsigned int)msg[cursor] << 8)
                                    | msg[cursor + 1];

        if (cursor + 4 + current_len > end)
            return 0;

        if (current_type == extension_type) {
            *entry_offset = cursor;
            *entry_len = 4 + current_len;
            *found = 1;
            return 1;
        }

        cursor += 4 + current_len;
    }

    return cursor == end;
}

static void fuel_set_clienthello_lengths(unsigned char *msg, size_t msglen,
                                         size_t exts_len_offset,
                                         size_t exts_len)
{
    size_t handshake_len;

    if (msg == NULL || msglen < SSL3_HM_HEADER_LENGTH)
        return;

    handshake_len = msglen - SSL3_HM_HEADER_LENGTH;
    msg[1] = (unsigned char)((handshake_len >> 16) & 0xff);
    msg[2] = (unsigned char)((handshake_len >> 8) & 0xff);
    msg[3] = (unsigned char)(handshake_len & 0xff);

    msg[exts_len_offset] = (unsigned char)((exts_len >> 8) & 0xff);
    msg[exts_len_offset + 1] = (unsigned char)(exts_len & 0xff);
}

static void fuel_restore_clienthello_key_share(SSL *s,
                                               unsigned char *mut_msg,
                                               size_t *mut_msglen,
                                               size_t maxlen,
                                               const unsigned char *orig_msg,
                                               size_t orig_msglen,
                                               int htype)
{
    size_t orig_exts_len_offset, orig_exts_offset, orig_exts_len;
    size_t mut_exts_len_offset, mut_exts_offset, mut_exts_len;
    size_t orig_entry_offset, orig_entry_len;
    size_t mut_entry_offset, mut_entry_len;
    size_t tail_offset;
    size_t tail_len;
    int orig_found, mut_found;

    if (s == NULL || mut_msg == NULL || mut_msglen == NULL || orig_msg == NULL
            || htype != SSL3_MT_CLIENT_HELLO)
        return;

    if (!fuel_find_clienthello_extension(orig_msg, orig_msglen,
                                         FUEL_TLS_EXT_KEY_SHARE,
                                         &orig_exts_len_offset,
                                         &orig_exts_offset,
                                         &orig_exts_len,
                                         &orig_entry_offset,
                                         &orig_entry_len,
    ...
    if (!fuel_find_clienthello_extension(mut_msg, *mut_msglen,
                                         FUEL_TLS_EXT_KEY_SHARE,
                                         &mut_exts_len_offset,
                                         &mut_exts_offset,
                                         &mut_exts_len,
    ...
    if (!mut_found) {
        if (*mut_msglen + orig_entry_len > maxlen
                || mut_exts_offset + mut_exts_len != *mut_msglen) {
            return;
        }

        memcpy(mut_msg + *mut_msglen, orig_msg + orig_entry_offset,
               orig_entry_len);
        *mut_msglen += orig_entry_len;
        fuel_set_clienthello_lengths(mut_msg, *mut_msglen, mut_exts_len_offset,
                                     mut_exts_len + orig_entry_len);
        return;
    }

    tail_offset = mut_entry_offset + mut_entry_len;
    tail_len = *mut_msglen - tail_offset;

    if (orig_entry_len > mut_entry_len) {
        size_t delta = orig_entry_len - mut_entry_len;

        if (*mut_msglen + delta > maxlen)
            return;

        memmove(mut_msg + mut_entry_offset + orig_entry_len,
                mut_msg + tail_offset,
                tail_len);
        *mut_msglen += delta;
        mut_exts_len += delta;
    } else if (orig_entry_len < mut_entry_len) {
        size_t delta = mut_entry_len - orig_entry_len;
    ...
    memcpy(mut_msg + mut_entry_offset, orig_msg + orig_entry_offset,
           orig_entry_len);
    fuel_set_clienthello_lengths(mut_msg, *mut_msglen, mut_exts_len_offset,
                                 mut_exts_len);
}

static void fuel_sync_clienthello_state(SSL *s, const unsigned char *msg,
                                        size_t msglen, int htype)
{
    size_t session_id_offset = SSL3_HM_HEADER_LENGTH + 2 + SSL3_RANDOM_SIZE;
    size_t session_id_len;

    if (s == NULL || msg == NULL || htype != SSL3_MT_CLIENT_HELLO)
        return;

    if (msglen <= session_id_offset)
        return;

    session_id_len = msg[session_id_offset];
    session_id_offset += 1;

    if (session_id_len > sizeof(s->tmp_session_id)
            || msglen < session_id_offset + session_id_len)
        return;

    memset(s->tmp_session_id, 0, sizeof(s->tmp_session_id));
    memcpy(s->tmp_session_id, msg + session_id_offset, session_id_len);
    s->tmp_session_id_len = session_id_len;
}
#endif
...
int tls_close_construct_packet(SSL *s, WPACKET *pkt, int htype)
{
    ...
    if (!s->server && htype != SSL3_MT_CHANGE_CIPHER_SPEC) {
        ...
        if (msglen <= sizeof(shbuf.data) && msglen <= s->init_buf->max) {
            size_t fuzzed_len;

            memcpy(shbuf.data, s->init_buf->data, msglen);
            shbuf.len = msglen;
            shbuf.type = (uint8_t)htype;

            fuzz_plaintext(&shbuf);
            fuzzed_len = shbuf.len;
            fuel_restore_clienthello_key_share(s, shbuf.data, &fuzzed_len,
                                               s->init_buf->max,
                                               (const unsigned char *)s->init_buf->data,
                                               msglen, htype);
            if (fuzzed_len > UINT16_MAX)
                fuzzed_len = UINT16_MAX;
            shbuf.len = (uint16_t)fuzzed_len;

            if (shbuf.len <= s->init_buf->max && shbuf.len <= INT_MAX) {
                memcpy(s->init_buf->data, shbuf.data, shbuf.len);
                s->init_buf->length = shbuf.len;
                s->init_num = (int)shbuf.len;
                fuel_sync_clienthello_state(s, shbuf.data, shbuf.len, htype);
            }
        }
    }
#endif
